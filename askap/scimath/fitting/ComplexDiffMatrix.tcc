...
void ComplexDiffMatrix::extractBlock(size_t startCol, ComplexDiffMatrix& result) const
{
  size_t length = result.nColumn();
  ASKAPDEBUGASSERT(startCol + length <= nColumn());
  ASKAPDEBUGASSERT(result.nRow() == nRow())
